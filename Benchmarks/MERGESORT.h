...
template <class TIter, class TCmp>
void Sort(TIter res_begin, TIter left, TIter right, TCmp cmp){
    if (std::distance(left, right)<=1) {return;}

    TIter middle = left + std::distance(left, right)/2;
    Sort(res_begin, left, middle, cmp);
    Sort(res_begin + std::distance(left, middle), middle, right, cmp);
