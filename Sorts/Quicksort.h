...
uint64_t GetRandom(const uint32_t &n) {
...
TIter Random(TIter begin, TIter end, TCmp cmp={}){
    return begin + GetRandom(std::distance(begin, end));
}
template <class TIter, class TCmp=std::less<typename std::iterator_traits<TIter>::value_type>>
TIter Median(TIter begin, TIter end, TCmp cmp={}){
    srand(time(NULL));
    size_t size = std::distance(begin, end);
    TIter a = begin + GetRandom(size);
    TIter b = begin + GetRandom(size);
    TIter c = begin + GetRandom(size);
    if (!cmp(*a, *b) && !cmp(*c, *a)){
        return a;}
    if (!cmp(*b, *a) && !cmp(*c, *b)){
        return b;}
